...
function analyzeContentMatch(live, template, procedureName) {
  ...
  live.headings.forEach((heading) => {
    if (
      template.headings.some((th) => th.includes(heading) || heading.includes(th.substring(0, 15)))
    ) {
  ...
  live.paragraphs.forEach((paragraph) => {
    ...
    if (
      paragraph.includes("Starting Price") ||
      paragraph.includes("$") ||
      paragraph.includes("required fields") ||
      paragraph.length < 50
    ) {
      return;
    }
    ...
    if (templateText.includes(firstWords)) {
      ...
      if (
        paragraph.toLowerCase().includes(procedureName.replace("-", " ")) ||
  ...
  const relevantLiveContent = live.paragraphs.filter(
    (p) =>
      p.length > 50 &&
