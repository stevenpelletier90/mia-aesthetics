...
async function verifyAllProcedures() {
  ...
  for (const [name, config] of Object.entries(PROCEDURES)) {
    ...
    // Display results
    console.log(
      `Live site: ${liveContent.headings.length} headings, ${liveContent.paragraphs.length} paragraphs`
    );
    console.log(
      `Template: ${templateContent.headings.length} headings, ${templateContent.paragraphs.length} paragraphs`
    );
