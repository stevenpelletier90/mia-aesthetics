...
(function() {
  ...
  window.initGoogleMapsCareers = function() {
    ...
    if (typeof initializeAutocompleteCareers === 'function') {
  ...
  async function loadCareerLocations() {
    ...
    try {
      ...
      const response = await fetch('/wp-json/wp/v2/pages?per_page=100&meta_key=_wp_page_template&meta_value=page-careers-locations.php');
      if (!response.ok) throw new Error('Failed to load career locations');
      ...
      const locationsWithCoordinates = careerLocations.filter(loc => loc.coordinates !== null);
  ...
  setTimeout(() => {
    if (typeof google !== 'undefined' && google.maps && typeof initializeAutocompleteCareers === 'function') {
      console.log('Google Maps loaded but callback not called for careers, initializing manually...');
      initializeAutocompleteCareers();
    }
  }, 3000);
